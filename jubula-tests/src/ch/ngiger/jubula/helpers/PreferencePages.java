...
public class PreferencePages extends Common {
	...
	public void visit_all_preferencePages() throws Exception{
		...
		try {
			...
			while (true) {
				...
				AUT_run.takeScreenshotActiveWindow(m_aut, m_app,
					"preferences/page_" + new_pos + ".png"); //$NON-NLS-1$ //$NON-NLS-2$
				clickComponent(ok_btn);
				while (true) {
			...
			AUT_run.takeScreenshotActiveWindow(m_aut, m_app, "preferences/exception.png"); //$NON-NLS-1$
		...
		AUT_run.takeScreenshotActiveWindow(m_aut, m_app, "preferences/last_page.png"); //$NON-NLS-1$
